...
function validateRootfsSize() {
  const rootfsSizeInput = $("#rootfs-size");
  const value = parseInt(rootfsSizeInput.value);

  if (isNaN(value) || value < 0 || value > 1024) {
    showAlert("RootFS size must be between 0 and 1024 MB.");
    return false;
  }
...
function buildAsuRequest(request_hash) {
  ...
  var body = JSON.stringify({
    ...
    rootfs_size_mb: parseInt($("#rootfs-size").value) || 0, // 添加 rootfs_size_mb
  });
  var method = "POST";
